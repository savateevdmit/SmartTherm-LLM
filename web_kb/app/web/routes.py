...
def _insert_question_with_vector(db: Session, text_val: str, tags_val: Optional[str], emb: Optional[List[float]]) -> int:
    """
    Insert question and write VECTOR column using VEC_FromText(:emb_json).
    embedding_json column does NOT exist.
    Returns new question id.
    """
    emb_json = _emb_to_json_str(emb)
    if emb_json is None:
        # If you want to require embedding always, you can raise.
        # We'll still insert with NULL embedding.
        stmt = sql_text(
            "INSERT INTO questions (text, tags) VALUES (:text, :tags)"
        )
        db.execute(stmt, {"text": text_val, "tags": tags_val})
    else:
        stmt = sql_text(
            "INSERT INTO questions (text, embedding, tags) VALUES (:text, VEC_FromText(:emb_json), :tags)"
        )
...
def _update_question_with_vector(db: Session, qid: int, text_val: str, tags_val: Optional[str], emb: Optional[List[float]]):
    ...
    if emb_json is None:
        stmt = sql_text(
            "UPDATE questions SET text=:text, tags=:tags, embedding=NULL WHERE id=:id"
        )
        db.execute(stmt, {"id": qid, "text": text_val, "tags": tags_val})
    else:
        stmt = sql_text(
            "UPDATE questions SET text=:text, tags=:tags, embedding=VEC_FromText(:emb_json) WHERE id=:id"
        )
...
def _normalize_files(files: Union[UploadFile, List[UploadFile], None]) -> List[UploadFile]:
    """
    Browser/Starlette may send:
    - a single UploadFile instead of list (even with `multiple`)
    - an "empty" UploadFile when nothing selected (filename="")
    We normalize to a list and drop empties.
    """
    if files is None:
        return []
...
def login_page(request: Request, db: Session = Depends(get_db), next: str = Query(default="/questions")):
    admins_count = db.scalar(select(func.count()).select_from(Admin)) or 0
    return templates.TemplateResponse("login.html", {"request": request, "admins_count": admins_count, "next": next})
...
def login_action(
    ...
    if not admin or not verify_password(password, admin.password_hash):
        ...
        return templates.TemplateResponse(
            "login.html",
            {"request": request, "error": "Неверный логин или пароль.", "admins_count": admins_count, "next": next},
            status_code=400,
        )

    token = sessions.create(SessionData(admin_id=admin.id, username=admin.username, role=admin.role, issued_at=now_ts()))
    resp = _redirect(next or "/questions")
    resp.set_cookie(
        key=get_cookie_name(),
        value=token,
        httponly=True,
...
def admins_list(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "admins_list.html",
        {"request": request, "sess": sess, "admins": admins, "admins_admin_count": admins_admin_count},
    )
...
async def question_new_action(
    ...
    return _redirect(f"/questions/{qid}")
...
async def question_edit_action(
    ...
    # update question via raw SQL when needed
    try:
        if text_changed:
            emb = await _embed_with_timeout(text, seconds=30)
        else:
            emb = None  # do not overwrite embedding if text not changed

        if text_changed:
            _update_question_with_vector(db, question_id, text, tags_csv or None, emb)
        else:
            # update only text/tags without touching embedding
    ...
    # reload question (text/tags might changed)
    q_obj = db.get(Question, question_id)
...
def answer_edit_redirect(question_id: int):
    return _redirect(f"/questions/{question_id}/edit")
